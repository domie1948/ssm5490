...
export default {
  ...
  computed: {
    zongjia:{
      get: function () {
        return 1*this.ruleForm.piaojia*this.ruleForm.shuliang
      }
    },
  },
  created() {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yingpiandingdan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yingpiandingdanCrossAddOrUpdateFlag = false;
                  this.parent.search();
